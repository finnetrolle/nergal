...
def record_to_blood_pressure_measurement(record: Record) -> BloodPressureMeasurement:
    """Convert a database record to BloodPressureMeasurement model."""
    return BloodPressureMeasurement(
        id=record["id"],
        user_id=record["user_id"],
        measured_at=record["measured_at"],
        systolic_1=record["systolic_1"],
        diastolic_1=record["diastolic_1"],
        systolic_2=record["systolic_2"],
        diastolic_2=record["diastolic_2"],
        systolic_3=record["systolic_3"],
        diastolic_3=record["diastolic_3"],
        systolic_avg=record["systolic_avg"],
        diastolic_avg=record["diastolic_avg"],
        notes=record["notes"],
...
class HealthMetricsRepository:
    """Repository for health metrics CRUD operations."""
    ...
    async def create_blood_pressure_measurement(
        ...
        Args:
            user_id: Telegram user ID.
            measured_at: Time of measurement (UTC).
            systolic_1: First systolic reading (mmHg).
            diastolic_1: First diastolic reading (mmHg).
            systolic_2: Second systolic reading (mmHg).
            diastolic_2: Second diastolic reading (mmHg).
            systolic_3: Third systolic reading (mmHg).
            diastolic_3: Third diastolic reading (mmHg).
            notes: Optional notes.
            user_timezone: User's timezone at time of measurement.

        Returns:
            Created BloodPressureMeasurement instance.
        """
        # Calculate averages
        systolic_avg = (systolic_1 + systolic_2 + systolic_3) / 3
        diastolic_avg = (diastolic_1 + diastolic_2 + diastolic_3) / 3

        query = """
            INSERT INTO blood_pressure_measurements (
                user_id, measured_at,
                systolic_1, diastolic_1,
                systolic_2, diastolic_2,
                systolic_3, diastolic_3,
        ...
        return record_to_blood_pressure_measurement(record)

    async def get_blood_pressure_by_id(
        self,
        measurement_id: UUID,
        user_id: int | None = None,
        ...
        Args:
            ...
            user_id: Optional user ID for ownership verification.
                If provided, only returns the measurement if it belongs to this user.

        Returns:
            BloodPressureMeasurement instance or None if not found.
        """
        if user_id is not None:
            query = "SELECT * FROM blood_pressure_measurements WHERE id = $1 AND user_id = $2"
            record = await self._db.fetchrow(query, measurement_id, user_id)
        else:
            query = "SELECT * FROM blood_pressure_measurements WHERE id = $1"
            record = await self._db.fetchrow(query, measurement_id)
        return record_to_blood_pressure_measurement(record) if record else None

    async def get_blood_pressure_for_user(
        self,
        user_id: int,
        limit: int = 30,
        offset: int = 0,
    ) -> list[BloodPressureMeasurement]:
        """Get blood pressure measurements for a user.
        ...
        Returns:
            List of BloodPressureMeasurement instances.
        """
        query = """
        ...
        """
        records = await self._db.fetch(query, user_id, limit, offset)
        return [record_to_blood_pressure_measurement(r) for r in records]

    async def get_blood_pressure_for_date_range(
        self,
        user_id: int,
        start_date: datetime,
        end_date: datetime,
    ) -> list[BloodPressureMeasurement]:
        """Get blood pressure measurements for a user within a date range.
        ...
        query = """
            ...
            ORDER BY measured_at DESC
        """
        records = await self._db.fetch(query, user_id, start_date, end_date)
        return [record_to_blood_pressure_measurement(r) for r in records]

    async def get_blood_pressure_by_day(
        self,
        user_id: int,
        days: int = 7,
    ) -> dict[str, list[BloodPressureMeasurement]]:
        """Get blood pressure measurements grouped by day.
        ...
        Returns:
            Dictionary mapping date strings to lists of measurements.
        """
        query = """
            SELECT * FROM blood_pressure_measurements
        ...
        measurements = [record_to_blood_pressure_measurement(r) for r in records]

        # Group by date
        result: dict[str, list[BloodPressureMeasurement]] = {}
        for m in measurements:
            date_key = m.measured_at.strftime("%Y-%m-%d")
            if date_key not in result:
                result[date_key] = []
            result[date_key].append(m)
    ...
    async def get_blood_pressure_stats(
        ...
        query = """
            SELECT
                ...
                AVG(systolic_avg) as avg_systolic,
                AVG(diastolic_avg) as avg_diastolic,
                MIN(systolic_avg) as min_systolic,
                MAX(systolic_avg) as max_systolic,
        ...
        return {
            "total_measurements": record["total_measurements"] or 0,
            "avg_systolic": round(record["avg_systolic"], 1) if record["avg_systolic"] else None,
            "avg_diastolic": round(record["avg_diastolic"], 1) if record["avg_diastolic"] else None,
            "min_systolic": record["min_systolic"],
            "max_systolic": record["max_systolic"],
            "min_diastolic": record["min_diastolic"],
            "max_diastolic": record["max_diastolic"],
        }

    async def delete_blood_pressure_measurement(
        self, user_id: int, measurement_id: UUID
    ) -> bool:
        ...
        """
        query = """
            DELETE FROM blood_pressure_measurements
            WHERE id = $1 AND user_id = $2
        """
        result = await self._db.execute(query, measurement_id, user_id)
        return result == "DELETE 1"
    ...
    async def get_latest_blood_pressure(
        ...
        Returns:
            Most recent BloodPressureMeasurement or None.
        """
        query = """
            SELECT * FROM blood_pressure_measurements
            WHERE user_id = $1
    ...
    @staticmethod
    def record_to_health_reminder(record: Record) -> HealthReminder:
        """Convert a database record to HealthReminder model."""
    ...
    async def create_reminder(
        ...
        Args:
            user_id: Telegram user ID.
            reminder_type: Type of reminder (e.g., "blood_pressure").
            reminder_time: Time for reminder in "HH:MM" format.
            user_timezone: User's timezone.

        Returns:
            Created HealthReminder instance.
        """
        query = """
            INSERT INTO health_reminders (
                user_id, reminder_type, reminder_time, user_timezone
            )
            VALUES ($1, $2, $3, $4)
            ON CONFLICT (user_id, reminder_type, reminder_time) DO UPDATE SET
                user_timezone = COALESCE(EXCLUDED.user_timezone, health_reminders.user_timezone),
                is_active = TRUE,
                updated_at = NOW()
            RETURNING id, user_id, reminder_type, reminder_time, user_timezone,
        ...
        return self.record_to_health_reminder(record)
    ...
    async def get_reminders_for_user(
        ...
        Args:
            user_id: Telegram user ID.
            reminder_type: Optional filter by type.
            active_only: Only return active reminders.

        Returns:
            List of HealthReminder instances.
        """
        # Build queries without string interpolation to avoid SQL injection risks
        if reminder_type:
            if active_only:
                query = """
                    SELECT * FROM health_reminders
                    WHERE user_id = $1 AND reminder_type = $2 AND is_active = TRUE
                ...
                query = """
                    ...
                    ORDER BY reminder_time
                """
            records = await self._db.fetch(query, user_id, reminder_type)
        else:
            if active_only:
                query = """
                    SELECT * FROM health_reminders
                    WHERE user_id = $1 AND is_active = TRUE
                    ORDER BY reminder_type, reminder_time
                """
            else:
                query = """
                    SELECT * FROM health_reminders
        ...
        return [self.record_to_health_reminder(r) for r in records]
    ...
    async def delete_reminder(
        ...
        """
        result = await self._db.execute(query, user_id, reminder_type, reminder_time)
        return result == "DELETE 1"
